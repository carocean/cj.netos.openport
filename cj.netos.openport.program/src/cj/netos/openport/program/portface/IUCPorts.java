...
public interface IUCPorts extends IOpenportService {
    ...
    @CjOpenport(usage = "测试2", reciever = MyOpenportContentReciever.class, tokenIn = TokenIn.headersOfRequest, acl = {"allow *.role"}, command = "post", type = LinkedList.class, beforeInvoker = TestInvoker.class, afterInvoker = TestInvoker.class)
    ...
    class TestInvoker implements IOpenportBeforeInvoker, IOpenportAfterInvoker {
        @Override
        public void doAfter(String methodName, CjOpenport openport, Frame frame, Circuit circuit) {
            CJSystem.logging().info(getClass(),String.format("doAfter: %s | %s",methodName,frame));
        }

        @Override
        public void doBefore(String methodName, CjOpenport openport, TokenInfo tokenInfo, Frame frame, Circuit circuit) {
            CJSystem.logging().info(getClass(),String.format("doBefore: %s | %s | %s",methodName,tokenInfo,frame));
        }
    }
}
